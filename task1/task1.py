...
# Ранее мы рассматривали (и строили в тетрадках) деревья разбора (или деревья вывода --- синоним),
# которые показывают, как некоторая строка языка выводится из аксиомы. Дерево абстрактного синтаксиса
# содержит только содержательную информацию о программе --- в нём нет разделителей (запятые, точки
# с запятой, скобки...), вспомогательных нетерминалов для указания приоритета и т.д.
#
# Абстрактное синтаксическое дерево --- это структура данных в программе


# Пример. Для арифметических выражений синтаксическое дерево может быть описано как
...
# Давайте теперь будем разбирать небольшую программу. Программа у нас будет последовательностью
# операторов, среди которых будут оператор ввода input, оператор присваивания и оператор печати print.
# Пускай операторы будут разделяться точкой с запятой.
#
# input x, y;
# z = (x + y) / 2;
# print x, y, z, 2+2
...
## WhileStmt = 'while' CondExpr 'do' Program 'end'.
# Program = Statement { ';' Statement }.
# Statement = Input | Assign | Print | IfStmt.
# Input = 'input' VARIABLE { ',' VARIABLE }.
# Assign = VARIABLE '=' Expr.
# Print = 'print' Expr {',' Expr}.
# IfStmt = 'if' CondExpr 'then' Program 'else' Program 'end'.
...
# ДЗ.
# 1. Сделать ветку else в условном операторе необязательной:
#
# IfStmt = 'if' CondExpr 'then' Program [ 'else' Program ] 'end'.
#
# 2. Сделать оператор цикла:
# 
# Statement = Input | Assign | Print | IfStmt | WhileStmt.
# WhileStmt = 'while' CondExpr 'do' Program 'end'.
